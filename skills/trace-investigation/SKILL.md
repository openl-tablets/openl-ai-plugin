...
6. `openl_export_trace` dumps the whole run as plain text — useful for a small run or
   a shareable artifact; `release: true` also frees the trace from server memory.
   `openl_cancel_trace` aborts a run that is taking too long.

Watch the response size on a large project: prefer walking the branch you need over
exporting a whole rating run.
...
1. Start with a whole-run profiling pass: `openl_start_trace` with
   `stopAtEntry: false`, `profiling: true` and no breakpoints. The run completes in
   that single call and returns a constant-size `profile` — `hotspots` (slowest
   tables by selfMillis/totalMillis/count) plus `nodeCount`/`distinctTables`/
   `totalMillis`. Add `includeTree: true` to browse structure, and walk deeper levels
   with `openl_expand_trace_tree`.
2. Scan `profile.hotspots` for the table relevant to the problem — the hot table, or
   one that ran an unexpected number of times.
3. `openl_watch_trace_cells` returns one series per named cell across the whole run,
   ...
   outlier (e.g. `83.372` among `1.0`s).
...
- **404** — the project session is stale. Open the project, then retry. Do NOT fall
  back to manual table reading; 404 is a session issue.
- **Still failing after opening the project** — stop, report the exact error message,
  do not attempt manual reconstruction, and suggest filing a bug against the OpenL
  MCP server.
...
apply without explicit user confirmation.** A table edit lives in the project's
working copy until it is saved — finish by saving, and say so.
