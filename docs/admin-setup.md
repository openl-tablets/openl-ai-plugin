...
as absent, rather than sent as an empty credential, since `1.1.0`. The separate desktop /
...
1. **Pre-provision via settings** — add the marketplace and enable the plugin in a
   ...
   **`autoUpdate` is what makes deployed installations follow new releases.** Claude
   ...
   pass your own validation first — then plan to push updates yourself. Three caveats:
   the setting scope that defines `autoUpdate` becomes its owner, so declaring it in
   managed settings also takes the per-marketplace **Enable auto-update** toggle in
   `/plugin` away from users (in an editable scope they can flip it themselves, and will
   if you don't); an update applies to the next session, not the running one; and the
   flag has no effect inside the Claude **desktop app**, which starts its bundled Claude
   Code with the auto-updater switched off (checked through Claude Code 2.1.222).
...
- **Version policy**: the analyst guide uses
  ...
  `npx -y -p openl-mcp@1.2.0 openl-mcp`; pinned installations update only when you
...
- **Authentication modes.** Multi-user Studio uses the explicit PAT setting;
  ...
  credential. Compatibility note: the pinned `openl-mcp@1.2.0` removed the fallback to
  a PAT cached by an older direct CLI sign-in; `1.1.0` still falls back to it when the
  explicit setting is absent, which matters for a desktop/Cowork config held on that
  version. The plugin never creates or manages that cache, and it is not a supported
