...
- Codex support: a native `.codex-plugin/plugin.json` (MCP server key `openl-ai`, skills, `interface`) plus a bundled configurator (`scripts/configure-codex.mjs`) and launcher (`scripts/start-openl-mcp-codex.mjs`). The configurator stores the Studio address and Personal Access Token in a private user config file with no `--token` argument and echo-off entry; the launcher injects them into an isolated `openl-mcp` process. Loopback HTTP remains supported for local Studio copies, including PAT authentication, with an unencrypted-transport warning. Non-loopback HTTP requires an explicit `--allow-insecure` / `OPENL_AI_ALLOW_INSECURE=1` opt-in. Analyst guide in `docs/codex-setup.md`.
- A `node --test` suite and GitHub Actions workflow covering manifests, config safety, HTTP policy, launcher isolation, and MCP pin parity.
